...
/* ── Panoramic background image ───────────────────────────────────────────────
   Fallback used when no dynamic config is saved in Firestore yet.
───────────────────────────────────────────────────────────────── */
...
export function CharacterHotspot({ config }: CharacterHotspotProps = {}) {
  // If an admin-configured config exists and is explicitly disabled, hide section
  ...
  const hotspots: HotspotPin[] =
    config?.pins && config.pins.length > 0 ? config.pins : HOTSPOTS;

  return <CharacterHotspotInner
...
function CharacterHotspotInner({
  ...
  return (
    ...
    >
      {/* Panoramic scene — 16:9 container matches the 1600×900 image exactly so
           object-cover never crops and all pin % coords map 1:1 to the image. */}
      <div
        ref={containerRef}
        className="relative w-full"
        style={{ aspectRatio: "16/9", minHeight: "420px" }}
      >
        {/* Header overlay — gradient backdrop for legibility */}
        <div
        ...
        {/* Image layer — contained so image stays within bounds */}
        <div className="absolute inset-0 overflow-hidden">
          <Image
            ...
            priority={false}
          />
          {/* Layered dark gradient overlays for depth + legibility */}
          <div
        ...
        {/* Universe zone labels — below overlaid header */}
        <div
      ...
      {/* Mobile bottom-sheet popup — fixed overlay when a pin is tapped */}
      {isMobile && active && (
        <div
      ...
      >
        {(
          ...
          ] as { label: string; href: string; color: string; icon: string }[]
