...
type FranchiseFormData = Franchise;

interface FranchiseFormProps {
  initial?: Partial<FranchiseFormData>;
  onSubmit: (data: FranchiseFormData) => Promise<void>;
  submitLabel?: string;
}

export function FranchiseForm({ initial, onSubmit, submitLabel = "Save" }: FranchiseFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Input
        ...
        placeholder="Circular tile — /franchise index and homepage strip"
      />
      <Input
        label="Banner Image URL"
        value={bannerImage}
        onChange={(e) => setBannerImage(e.target.value)}
        placeholder="Full-width header on /franchise/[slug]"
      />
      <div>
