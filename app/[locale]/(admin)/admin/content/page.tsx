...
export default function AdminContentPage() {
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold" style={{ color: 'var(--color-black)' }}>Content</h1>
      <div className="grid gap-4 sm:grid-cols-2">
        {contentSections.map((s) => (
          <Link key={s.href} href={s.href} className="rounded-md border p-4 hover:border-red-300 transition-colors" style={{ background: 'var(--surface-elevated)', borderColor: 'var(--border-ink)' }}>
            <p className="font-semibold" style={{ color: 'var(--color-black)' }}>{s.label}</p>
            <p className="text-sm mt-1" style={{ color: 'var(--color-muted)' }}>{s.description}</p>
          </Link>
        ))}
      </div>
    </div>
